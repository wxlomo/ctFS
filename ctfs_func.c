...
ssize_t  ctfs_pread(int fd, void *buf, size_t count, off_t offset){
	...
	if(offset >= ct_rt.fd[fd].inode->i_size){
		...
		int res = ctfs_file_range_lock_release(fd, current_fl);
		if(res){
			ct_rt.errorn = res;
			return -1;
		}
...
#endif
	int res = ctfs_file_range_lock_release(fd, current_fl); // release read lock
	if(res){
		ct_rt.errorn = res;
		return -1;
	}
...
static inline ssize_t  ctfs_pwrite_normal(int fd, const void *buf, size_t count, off_t offset){
	...
	ct_rt.fd[fd].cpy_time += timer_end();
#endif
	int res = ctfs_file_range_lock_release(fd, current_fl);  // release write lock
	if(res){
		ct_rt.errorn = res;
		return -1;
	}
